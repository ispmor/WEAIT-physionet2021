...
def evaluate_model(label_directory, output_directory):
    ...
    max_scalars = [ max(s) for s in scalar_outputs ]
    np.savetxt("max_scalars.csv", max_scalars)
    mean_max_scalars = np.median(max_scalars)
    percentile_ninety = np.percentile(max_scalars, 90)
    print("MDIANE MAX SCALARS: ", mean_max_scalars)
    print("90 percentile: ", percentile_ninety)
